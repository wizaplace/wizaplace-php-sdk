...
final class BannerServiceTest extends ApiTestCase
{
    ...
    public function testGetBannersWithAltTags()
    {
        $cmsService = $this->buildBannerService();

        $banners1 = $cmsService->getHomepageBanners();
        $this->assertSame(2, count($banners1));
        $this->assertSame(8, $banners1[0]->getImageId());
        $this->assertSame("Texte alternatif image", $banners1[0]->getAlt());

        $this->assertSame('http://url-de-test-2.com', (string) $banners1[1]->getLink());
        $this->assertSame(9, $banners1[1]->getImageId());
        $this->assertFalse($banners1[1]->getShouldOpenInNewWindow());

        $banners2 = $cmsService->getCategoriesBanners(3, 'desktop');
        $this->assertSame(1, count($banners2));
        $this->assertSame(9, $banners2[0]->getImageId());
        $this->assertSame("Texte alternatif image", $banners2[0]->getAlt());
    }
